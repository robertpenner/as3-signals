...
h3. v0.8 - Maximilian - 2010-11-__

h4. API Changes
* Signals are now MXML-friendly! E.g:
  signals:Signal id="nameChanged">{[String, uint]}</signals:Signal>
  
* Now writable:
** valueClasses (Signal, DeluxeSignal)
** eventClass (NativeSignal, NativeRelaySignal)
...
** INativeSignalOwner: IPrioritySignal, INativeDispatcher, adds removeAll().
** Both these interfaces are needed because dispatch(event:Event) conflicts with dispatch(...valueObjects).
** Thanks to "Brian Heylin":http://github.com/brianheylin for getting the ball rolling.

h4. Fixes
* Changing NativeSignal.target wasn't removing listeners from target.
